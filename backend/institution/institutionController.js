...
find = async (req, res) => {
  ...
  try {
    ...
    const filtered = found.filter((e) => {
      if (String(e.ins_code).toLowerCase().includes(String(input).toLowerCase())) return true
      if (String(e.name).toLowerCase().includes(String(input).toLowerCase())) return true
      if (String(e.address).toLowerCase().includes(String(input).toLowerCase())) return true
